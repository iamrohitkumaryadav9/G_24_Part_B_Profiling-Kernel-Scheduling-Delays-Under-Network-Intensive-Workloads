...
Hypothesis H1:
  When softirq processing is co-located on the same CPUs as application threads
  (default behavior), scheduling delay increases due to CPU contention.
  Steering softirq to dedicated CPUs (via RPS) should reduce tail latency.
...
def compute_concentration_metrics(per_cpu_values):
    ...
    Gini coefficient ranges from 0 (perfectly uniform) to 1 (all on one CPU).
    A high Gini means softirq work is heavily concentrated on few cores, which
    typically correlates with higher scheduling jitter on those cores.
    ...
    # where x_i are the sorted fractions and i is 1-indexed rank.
    n = len(sorted_fracs)
    index = np.arange(1, n + 1)
    gini = (2 * np.sum(index * sorted_fracs)) / (n * np.sum(sorted_fracs)) - (n + 1) / n

    # Fraction of total handled by the single busiest CPU
    max_frac = np.max(fractions)
    # Combined fraction handled by the top-3 busiest CPUs
    top3_frac = np.sum(np.sort(fractions)[-3:])
    # Coefficient of variation: std/mean — high CV means uneven load
