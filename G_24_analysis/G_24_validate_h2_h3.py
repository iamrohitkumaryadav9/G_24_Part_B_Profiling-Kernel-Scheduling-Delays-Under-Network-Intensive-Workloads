...
H2 — ksoftirqd Hypothesis:
  Forcing NET_RX processing into a dedicated ksoftirqd thread (E10, via reduced
  NAPI budget) should reduce scheduling jitter for application threads because
  softirq work is offloaded from the run-queue of the app CPU.

H3 — TCP vs UDP Protocol Hypothesis:
  UDP bypasses the TCP retransmit and ACK machinery, so per-packet scheduling
  overhead should be lower under the same CPU+network stress (E11/E12 vs E1/E4).

E13 is also analysed here to locate the CPU-contention inflection point where
p99 latency first crosses the 1 ms boundary.
...
def parse_softnet_stat(filepath):
    ...
    Each row records cumulative counters for one CPU at one timestamp.
    Values are stored as hex strings matching the kernel's /proc/net/softnet_stat
    format.  We return only the LAST timestamp row per CPU so callers get the
    final cumulative snapshot of the experiment.
    ...
    with open(filepath, 'r') as f:
        ...
        for row in reader:
            ...
            try:
                cpu = int(row[1])
                # Kernel writes these as hex; convert to decimal for arithmetic
...
def plot_h2_comparison(data_dir, output_dir, num_cpus=20):
    ...
    # Use a twin y-axis so time_squeeze (left) and voluntary switches (right)
    # share the same x-axis without one metric dominating the scale of the other.
    ...
    # Place raw count above each bar as a label; offset by 2% of the axis range
    # so the text never overlaps the bar top even when bars are very short.
