...
Shows three correlated signals over time for each key experiment:
  - Network throughput (Gbps) from iperf3
  - Scheduling delay p50/p99 (μs) from sampled sched_delay.csv events
  - Softirq CPU utilization (%) from /proc/softirqs via cpu_util.csv

Outputs one PNG per experiment plus an overlay comparison and a
stress-progression grid (E1 → E13 → E4).
...
# Minimum samples required per bucket to compute a percentile
MIN_BUCKET_SAMPLES = 3

# Time-bucket width in seconds for per-second p50/p99 computation
BUCKET_SEC = 1.0

# Plot DPI for all saved figures
PLOT_DPI = 150
