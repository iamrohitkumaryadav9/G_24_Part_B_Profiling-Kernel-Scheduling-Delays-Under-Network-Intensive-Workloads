...
H4 — Combined Mitigations Hypothesis:
  Layering RPS spread + application CPU-pinning + CFS low-latency tuning +
  SO_BUSY_POLL should produce a measurable (≥20 %) reduction in p99 scheduling
  delay compared to the unmitigated E4 baseline.

  If p99 does not improve, we check voluntary context switches as a secondary
  signal: SO_BUSY_POLL should reduce blocking context switches even when CPU
  contention from stress-ng prevents overall p99 improvement.
...
def get_percentiles_from_file(filepath, percentiles=[50, 90, 95, 99, 99.9]):
    ...
    # Use only the last histogram snapshot (cumulative totals at experiment end)
    buckets = blocks[-1]
    ...
    for pct in percentiles:
        # Walk cumulative counts until we exceed the target quantile mass
        target = total * pct / 100.0
        cumsum = 0
        for low, high, count in buckets:
            cumsum += count
            if cumsum >= target:
                # Report the upper edge of the bucket that contains the percentile
                result[pct] = high
                break
        else:
...
def main():
    ...
    for exp in ['E14', 'E15', 'E16']:
        ...
        if exp_p99 is not None and e4_p99 is not None:
            # Require at least 20% reduction to claim a meaningful improvement
